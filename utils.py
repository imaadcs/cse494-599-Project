...
def write_blackbox_output_batchiter(loader, model, wf, device='cpu', ifscore=True):
    """
    Write predictions and optionally scores to a CSV file.
    Args:
        loader: DataLoader object
        model: Trained model
        wf: Opened CSV writer file
        device: 'cpu' or 'cuda' device
        ifscore: Whether to include scores in the output
    """
    import csv
    ...
    header = ['epi_embed', 'tcr_embed', 'true_label', 'predicted_label']
    if ifscore:
        header.append('score')
    ...
    for batch in loader['loader']:
        ...
        scores = model(X_pep, X_tcr).squeeze(-1).data.cpu().tolist()
        predictions = [round(s) for s in scores]

        for i in range(len(predictions)):
            row = [
                X_pep[i].cpu().tolist(),  # Embedding for epitope
                X_tcr[i].cpu().tolist(),  # Embedding for TCR
                int(y[i].item()),         # True label
                int(predictions[i])       # Predicted label
            ]
            if ifscore:
                row.append(float(scores[i]))  # Prediction score
            writer.writerow(row)
...
def get_performance_batchiter(loader, model, device='cpu'):
    ...
    for batch in loader:
        # Unpack the batch directly
        X_pep, X_tcr, y = batch
        X_pep, X_tcr, y = X_pep.to(device), X_tcr.to(device), y.to(device)

        yhat = model(X_pep, X_tcr).squeeze(-1)  # Ensure output is [batch_size]
        loss += F.binary_cross_entropy(yhat, y.float(), reduction='sum').item()
...
def get_performance(score, label):
    ...
    Args:
     score - 1D np.array or list of floats (predicted scores)
     label - 1D np.array or list of integers (true labels)
    '''

    # Convert scores to binary predictions
    label_pred = [round(s) for s in score]  # No indexing required
